    ...
    methods:{
        login: function () {
            AV.User.logIn(this.formData.username, this.formData.password).then(function (User) {
                ...
                //获取数据      
                var imassage = new AV.Query('IRusmer')
                imassage.descending('createdAt')
                imassage.include('IRusmer')
                imassage.include('IRusmer.targetAVUser')
                imassage.find().then(function(mess){
